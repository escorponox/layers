...
const staggeredBackwards = (backwardsLayers, previousLayer) => {
  previousLayer =
    previousLayer ||
    backwardsLayers[backwardsLayers.length - 1].previousElementSibling
...
const animateFrontLayers = layer =>
  slide(layer.parentNode.lastElementChild, 480, 300)
    .then(
      oldFrontLayer =>
        new Promise(resolve => {
    ...
    .then(backwardsLayers =>
      Promise.all(
        backwardsLayers.map((blayer, index) => {
...
const animateNewFrontLayer = layer => {
  ...
  return Promise.all([
    slide(layer, frontHeight, 500, true),
    slide(layer.parentNode, containerHeight, 500, true)
  ])
}
